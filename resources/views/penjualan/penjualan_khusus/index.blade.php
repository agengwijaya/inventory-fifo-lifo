...
@section('content')
      ...
      <div class="content__wrap">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item"><a href="#">Barang Keluar (FIFO)</a></li>
          </ol>
        </nav>
      </div>
    </div>
    ...
    <div class="content__wrap">
      <div class="row">
        @can('fifo.simpan')
          <div class="col-md-12">
            <div class="card h-100">
              <div class="card-body">
                @if ($mode_edit)
                  <a href="{{ url('fifo') }}" class="btn btn-warning btn-sm mb-3"><i class="fa fa-arrow-left me-2"></i>Kembali</a>
                @endif
                <h5 class="card-title">Formulir Barang Keluar (FIFO)</h5>
                <div class="row">
                  <div class="col-md-4">
                    <div class="card border-1 border-primary">
                      <div class="card-body">
                        <form action="{{ url('fifo') }}" method="POST" id="form_penjualan_khusus">
                          @csrf
                          <div class="row">
        ...
        <div class="col-md-12">
          <div class="card mt-3">
            <div class="card-body">
              <h5 class="card-title">List Barang Keluar (FIFO)</h5>
              <div class="row">
  ...
  <script>
    function modal_hapus(id, jenis) {
      $('#form_delete').attr('action', "{{ url('fifo/hapus?id=') }}" + id + '&jenis=' + jenis);
    }

    function modal_edit(id) {
      $.ajax({
        url: '{{ url('fifo/edit') }}' + '?id=' + id + '&customer_id=' + $('#customer_id').val(),
        method: "get",
        success: function(res) {
          $("#modal_edit_form").html(res);
        }
      })
      $('#form_edit_detail').attr('action', "{{ url('fifo') }}" + '/' + id);
    }
