...
class TransaksiKeluarLifoController extends Controller
{
    ...
    public function simpan_detail(Request $request)
    {
        ...
        $text = '';
        while ($sisa > 0) {
            $value_stok = Stok::where('jenis', 1)->where('barang_id', $request->barang_id)->where('stok_akhir', '!=', 0)->where('soft_delete', 0)->orderBy('tanggal_transaksi', 'desc')->orderBy('id', 'asc')->first();
